...
class CT(data.Dataset):
    ...
    def __init__(self, root, data_volume, task, split="train", transform=None):
        ...
        fileDescriptor = open(os.path.join("./datasets" + self.task, data_label), "r")
        ...
        while line:
            ...
            if line and not str(line).startswith("VolumeName") and not str(line).startswith("NoteAcc_DEID") \
                and not str(line).startswith("im") and not str(line).startswith("PatientID") and not str(line).startswith("SeriesInstanceUID"):
    ...
    def __getitem__(self, index):
        ...
        try:
            if self.task == "CT-Rate" or self.task == "CT-Rate_O":
            ...
            elif self.task == "CC-CCII" or self.task == "CC-CCII_new":
                imagePath = os.path.join(self.root + "images_new", imagePath)
                imageData = nib.load(imagePath)
                imageData = np.asanyarray(imageData.dataobj)
                ...
                imageData = nib.load(imagePath)
                imageData = np.asanyarray(imageData.dataobj)
            elif self.task == "RSPECT":
                imagePath = os.path.join(self.root + "images", imagePath + ".nii.gz")
                imageData = nib.load(imagePath)
                imageData = np.asanyarray(imageData.dataobj)
            elif self.task == "Stoic":
                imagePath = os.path.join(self.root + "images_preprocessed", imagePath + ".nii.gz")
                imageData = nib.load(imagePath)
                imageData = np.asanyarray(imageData.dataobj)
            elif self.task == "AHPH-10K":
                imagePath = os.path.join(self.root, imagePath)
                imageData = nib.load(imagePath)
                imageData = np.asanyarray(imageData.dataobj)
        except:
            print(imagePath)
