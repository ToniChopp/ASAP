...
def get_loader(args):
    ...
    transform_train_cc_ccii = Compose([
        AddChannel(),
        Orientation(axcodes="RAS", image_only=True),
        Spacing(
            pixdim=(args.spacing_x, args.spacing_y, args.spacing_z),
    ...
    transform_test_ccc_cii = Compose([
        AddChannel(),
        Orientation(axcodes="RAS", image_only=True),
        Spacing(
            pixdim=(args.spacing_x, args.spacing_y, args.spacing_z),
    ...
    if args.stage == "test":
        if args.task == "CC-CCII":
            test_set = CT(root = args.dataset_path, data_volume = args.data_volume, task=args.task, split="test", transform= transform_test_ccc_cii)
        else:
    ...
    if args.task == "CC-CCII":
        train_set = CT(root = args.dataset_path, data_volume = args.data_volume, task=args.task, split="train", transform= transform_train_cc_ccii)
        val_set = CT(root = args.dataset_path, data_volume = args.data_volume, task=args.task, split="val", transform= transform_test_ccc_cii)
    else:
