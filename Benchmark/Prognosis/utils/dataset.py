...
class CT(data.Dataset):
    ...
    def __init__(self, root, data_volume, task, split="train", transform=None):
        ...
        df = pd.read_csv(os.path.join("./datasets/" + self.task, data_label))
        self.df = df

    
    def _to_time_event(self, row, prefix):
        # prefix e.g. 'mort' or 'readmit'
        arr = [row[f"1_month_{prefix}"], row[f"6_month_{prefix}"], row[f"12_month_{prefix}"]]
        arr = [int(x) for x in arr]
        # find earliest event index
        if arr[0] == 1:
            return 0, 1
        elif arr[1] == 1:
            return 1, 1
            ...
            return 2, 0  # censored at last interval
    ...
    def __getitem__(self, index):
        ...
        if self.task == "INSPECT":    
            ...
            mort_t, mort_e = self._to_time_event(row, 'mortality')      # single ints
            # read_t, read_e = self._to_time_event(row, 'readmission')
            # ph_e = int(row['12_month_PH'])  # event (0/1) at 12m
            # ph_t = 2  # index of 12m (0,1,2)
            labels = row.iloc[1:4].astype(int).to_numpy()

            # targets = {}
            # targets['1m_mort'] = (torch.tensor(0, dtype=torch.float32),
            #                       torch.tensor(imageLabel[0], dtype=torch.float32))
            # targets['6m_mort'] = (torch.tensor(1, dtype=torch.float32),
            #                       torch.tensor(imageLabel[1], dtype=torch.float32))
            # targets['12m_mort'] = (torch.tensor(2, dtype=torch.float32),
            #                        torch.tensor(imageLabel[2], dtype=torch.float32))
        
            # targets['1m_readmit'] = (torch.tensor(0, dtype=torch.float32),
            #                         torch.tensor(imageLabel[3], dtype=torch.float32))
            # targets['6m_readmit'] = (torch.tensor(1, dtype=torch.float32),
            #                         torch.tensor(imageLabel[4], dtype=torch.float32))
            # targets['12m_readmit'] = (torch.tensor(2, dtype=torch.float32),
            #                         torch.tensor(imageLabel[5], dtype=torch.float32))
        
            # targets['12m_ph'] = (torch.tensor(0, dtype=torch.float32),
            #                     torch.tensor(imageLabel[6], dtype=torch.float32))
        
            if self.transform != None: imageData = self.transform(imageData)

            targets = {
                'survival_time_idx': torch.tensor(mort_t, dtype=torch.long),
                'survival_event': torch.tensor(mort_e, dtype=torch.float32),
                # 'readmit_time_idx': torch.tensor(read_t, dtype=torch.long),
                # 'readmit_event': torch.tensor(read_e, dtype=torch.float32),
                # 'ph_time_idx': torch.tensor(ph_t, dtype=torch.long),   # not really needed
                # 'ph_event': torch.tensor(ph_e, dtype=torch.float32),
                'data_value': torch.tensor(labels, dtype=torch.float32)
            }
        
        # Stoic
        else:
            imagePath = str(row["PatientID"])
            imagePath = os.path.join(self.root + "images_preprocessed", imagePath + ".nii.gz")
            imageData = nib.load(imagePath)
            imageData = np.asanyarray(imageData.dataobj)

            if self.transform != None: imageData = self.transform(imageData)
            targets = torch.tensor(row["probSevere"], dtype=torch.float32).unsqueeze(-1)

        return imageData, targets
