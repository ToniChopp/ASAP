...
#from encoder import SparseConvNeXtLayerNorm, _get_active_ex_or_ii
from modules.encoder import SparseConvNeXtLayerNorm, _get_active_ex_or_ii
...
class Hybird(nn.Module):
    def __init__(self,
        ...
        self.num_features = 216  # 32  #30  #36  #49  #36  #27
    ...
    def forward_encs(self, encs):
        ...
        outs = []
        for enc in encs:
            out = F.adaptive_avg_pool3d(enc, (6, 6, 6))
            outs.append(out)
    ...
    def forward(self, x):
        ...
        # print('out:', out.shape, out_avg.shape)
        return out, out_avg
...
class Hybird_Silding(nn.Module):
    def __init__(self,
                 in_channel: int = 3,
                 n_classes: int = 3,
                 channels: Tuple = (32, 64, 96, 128, 192),
    ...
    def forward(self, x):
        ...
        h, w, d = 96, 96, 96
        bs, C, H, W, D = x.shape
        gH, gW, gD = H // h, W // w, D // d

        x_patches = x.unfold(2, h, h).unfold(3, w, w).unfold(4, d, d)
        x_patches = x_patches.contiguous().view(bs, C, gH, gW, gD, h, w, d)
        x_patches = x_patches.permute(0,2,3,4,1,5,6,7).contiguous().view(-1, C, h, w, d)

        n_patches =x_patches.shape[0]
        feats_list = []
        #print(x_patches.shape)
        #for i in range(0, n_patches, sub_batch_size):
        #    patch_batch = x_patches[i : i + sub_batch_size]  # shape [sub_batch_size, C, z, y, x_]
        #    print(patch_batch.shape)
        #    x1, x2, x3, x4, x5 = self.embeddings(patch_batch)
        #    x5 = self.mae(x5, None)
        #    feats_list.append(F.adaptive_avg_pool3d(x5, (3, 3, 3)))
        #    del x1, x2, x3, x4, x5
        #    torch.cuda.empty_cache()
        
        x1, x2, x3, x4, x5 = self.embeddings(x_patches)
        x5 = self.mae(x5, None)
        #x5 = torch.cat(feats_list, dim=0)
        x5 = x5.view(bs, gD, gH, gW, 768, 6, 6, 6).permute(0,4,1,5,2,6,3,7).contiguous()
        x5 = x5.view(bs, 768, gD*6, gH*6, gW*6)
        
        out = F.adaptive_avg_pool3d(x5, (6, 6, 6))
...
def build_hybird_l(in_channel=1, n_classes=14, img_size=96):
    return Hybird(in_channel=in_channel,
                  n_classes=n_classes,
...
def build_hybird_l_sliding(in_channel=1, n_classes=14, img_size=96):
    return Hybird_Silding(in_channel=in_channel,
                  n_classes=n_classes,
                  channels=(48, 96, 192, 384, 768),
                  depths=(1, 1, 3, 3, 1),
                  kernels=(3, 3, 3, 3, 3),
...
if __name__ == '__main__':
    with torch.no_grad():
        x = torch.rand((4, 1, 192, 192, 192)).cuda()
        network = build_hybird_l_sliding().cuda()
        out, out_avg = network(x)
