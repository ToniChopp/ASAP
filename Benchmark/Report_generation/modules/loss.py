...
def compute_loss(output, reports_ids, reports_masks, labels=None, vis_label=None, txt_label=None,
    ...
    gen_loss = criterion(output[:, :-1], reports_ids[:, 1:], reports_masks[:, 1:]).mean()
    ...
    total_loss = gen_loss + 0.1 * label_loss + 0.1 * match_loss
    return total_loss, gen_loss, label_loss, match_loss
