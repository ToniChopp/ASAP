...
METRICS = ['BLEU_1', 'BLEU_2', 'BLEU_3', 'BLEU_4', 'CIDEr', 'ROUGE_L', 'Bert_Score']
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks, labels) in enumerate(t):
            ...
            total_loss, gen_loss, label_loss, match_loss = self.criterion(*((outputs[0],) + (reports_ids, reports_masks, labels) + outputs[1:] + (self.args,)))
            train_loss += total_loss.item()
            # gen_losses += gen_loss.item()
            # label_losses += label_loss.item()
            # match_losses += match_loss.item()
            total_loss.backward()
            torch.nn.utils.clip_grad_value_(self.model.parameters(), self.args.grad_clip)
            self.optimizer.step()

            t.set_description(f'total_loss:{total_loss.item():.3}')
            if self.args.test_steps > 0 and epoch > 1 and (batch_idx + 1) % self.args.test_steps == 0:
                # self.test_step(epoch, batch_idx + 1)
                # self.model.train()
                self.model.eval()
        # log = {'train_loss': train_loss / len(self.train_dataloader), 'gen_loss': gen_losses / len(self.train_dataloader),\
        #        'label_loss': label_losses / len(self.train_dataloader), 'match_loss': match_losses / len(self.train_dataloader)}
