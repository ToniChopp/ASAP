...
from modules.hybird_contrast3 import build_hybird_l as hybird_contrast_l
...
class VisualExtractor(nn.Module):
    def __init__(self, args):
        ...
        if args.visual_extractor == 'densenet':
        ...
        elif 'hyspark_contrast' in args.visual_extractor:
            spatial_dims = 3
            self.visual_extractor = args.visual_extractor
            #from .swin import Swin
            #self.model = Swin(in_channels=1, feature_size=args.feature_size, spatial_dims=spatial_dims)
            from .hybird_contrast3 import build_hybird_l_sliding as hybird_contrast_l
            
            self.model = hybird_contrast_l(in_channel=1, n_classes=1, img_size=96)
            pth = "/data1/fenghetang/hyspark_seg/pretrain_models/hyspark_contrast_large2.pth"
            model_dict = torch.load(pth, weights_only=False)
            self.model.load_state_dict(model_dict, strict=False)
            self.classifier = nn.Linear(args.d_vf, args.num_labels)
        elif 'vit' in args.visual_extractor and 'm3d' not in args.name:
            self.visual_extractor = args.visual_extractor
            from .vision_transformer import vit_base_patch16
            ...
            self.classifier = nn.Linear(args.d_vf, args.num_labels)
        elif 'vit' in args.visual_extractor and 'm3d' in args.name:
            self.visual_extractor = args.visual_extractor
            from .m3d_vit import m3d_vit
            self.model = m3d_vit(
                image_size=(args.roi_x, args.roi_y, args.roi_z),
                patch_size=(args.patch_size_x, args.patch_size_y, args.patch_size_z),
            )
    ...
    def forward(self, images):
        ...
        elif 'hyspark_contrast' in self.visual_extractor:
            #print(images.shape)
            patch_feats, avg_feats = self.model(images)
            labels = self.classifier(avg_feats)

        elif 'vit' in self.visual_extractor:
            patch_feats, avg_feats = self.model(images)
            labels = self.classifier(avg_feats)
