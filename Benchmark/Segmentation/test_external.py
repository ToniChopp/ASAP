...
def main():
    ...
    # wandb_init = False
    # wandb.init(
    #     project=f"Segmentation_external_{args.task}",
    ...
    # Test
    args.stage = "test"
    args.pretrained_path = os.path.join(args.output_dir, "model.pt")
    # args.pretrained_path = os.path.join(args.output_dir, "model_final.pt")
    # args.output_dir = os.path.join(args.output_dir, "test")
    args.max_epochs = 0
    if args.rank == 0:
        avg_dice, std_dice = main_worker(gpu=0, args=args, wandb_init=False)

        print("#########################")
        print("####### Final Results #######")
        print("External Test Dice: ", avg_dice)
        print("#########################")
    ...
    if (args.model_name is None) or args.model_name == "unetr":
        ...
        if args.stage == "test":
            ...
            if args.pretrained_path != "random":
                if "M3D" not in args.pretrained_path:
