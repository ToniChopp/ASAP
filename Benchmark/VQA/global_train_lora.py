...
LastEditors: xm_cmic
LastEditTime: 2024-05-25 20:57:50
FilePath: /src-0515/global_train.py
Description: 
...
from dataset.dataset_global import CTRATE_Dataset
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    num_training_steps: int = field(default = 1300)
...
class ModelArguments:
    llm_max_length: int = field(default = 512)
...
def get_qformer_params(model):
    return [
        p for n, p in model.named_parameters()
        if "qformer" in n
    ]

def get_projctor_params(model):
    return [
        p for n, p in model.named_parameters()
        if "image_proj_mlp" in n
    ]
...
def main():
    ...
    print("Setup Data")
    Train_dataset = CTRATE_Dataset(
        data_args.train_image_path_csv,
        data_args.train_regioned_report_csv,
        data_args.train_qa_abnormality_csv,
        data_args.train_qa_location_csv,
        data_args.train_qa_presence_csv,
        data_args.train_qa_size_csv,
        data_args.train_disorders_csv,
        data_args.train_mask_root_dir,
        data_args.train_anatomy_mask_root_dir,
        tokenizer=tokenizer,
        train=True,
        total_length=model_args.llm_max_length
    )

    lora_params = get_lora_params(model)
    qformer_params = get_qformer_params(model)
    proj_params = get_projctor_params(model)

    optimizer = AdamW(
        [
            {"params": proj_params, "lr": training_args.learning_rate},
    ...
    model.language_model.save_pretrained("lora_ckpt")
    # 保存其他模块（vision + projector）
    torch.save(
        {
            "vision": model.vision_backbone.state_dict(),
            "projector": model.image_proj_mlp.state_dict(),
        },
        "vision_proj.pth"
    )
