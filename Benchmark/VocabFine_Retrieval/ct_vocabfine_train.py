...
##        extractor_dict = extractor_dict['model']
        ...
        if args.name != "random":
            model_dict = dict(vit.state_dict())
            pretrained_dict = torch.load(args.vit_pretrained, map_location=torch.device('cpu'))
            if args.name == "m3d":
                pretrained_dict = pretrained_dict
            else:
                pretrained_dict = pretrained_dict['model']
            # vit.load_state_dict(pretrained_dict, strict=False)
            clip.visual_transformer.load_state_dict(pretrained_dict, strict=False)
    ...
    ds = CTReportDatasetinfer(args=args, data_folder=args.data_folder, csv_file=args.reports_file, mode="train", labels=args.labels)
    dl = DataLoader(ds, num_workers=8, batch_size=args.batch_size, shuffle=True)
    num_batches = len(dl)
...
##    for epoch in range(args.epochs):
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, batch in tqdm.tqdm(enumerate(dl)):
            ...
            pathologies = pathologies_all
            labels_tensor = labels_tensor_all
            B, L = labels_tensor.shape
            ...
            text_tokens = tokenizer(
                texts, return_tensors="pt", padding="max_length", truncation=True, max_length=512).to(torch.device('cuda'))

            text_feature, image_feature = model(text_tokens, inputs, device=torch.device('cuda'))
            ...
            logits = output.reshape(B, L, 2)
            logits = F.softmax(logits, dim=-1)
            labels = torch.zeros(B, L, 2).cuda()
            labels[:, :, 0] = labels_tensor
            labels[:, :, 1] = 1 - labels_tensor
            ...
            if i % args.print_every == 0:
                ...
                print(
                    ...
                    f"Loss: {np.mean(losses):.6f}\tlr: {get_lr(optimizer)}\tBatch (t) {batch_time:.3f}", flush=True
                )
